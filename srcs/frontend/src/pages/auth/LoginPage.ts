...
export class LoginPage extends Component<LoginPageProps, LoginPageState> {
  ...
  private async handleGoogleOAuth(): Promise<void> {
    ...
    try {
      ...
      console.log('🔍 Debug OAuth Environment:')
      console.log('VITE_OAUTH_ENABLED:', import.meta.env.VITE_OAUTH_ENABLED) // --- IGNORE ---
      console.log('VITE_GOOGLE_CLIENT_ID:', import.meta.env.VITE_GOOGLE_CLIENT_ID) // --- IGNORE ---
      console.log('VITE_GOOGLE_REDIRECT_URI:', import.meta.env.VITE_GOOGLE_REDIRECT_URI) // --- IGNORE ---
      
      // Check if OAuth is available
      const isAvailable = authService.isOAuthAvailable()
      console.log('🔍 OAuth Available:', isAvailable)
      
      if (!isAvailable) {
        throw new Error('Google OAuth is not available. Please check configuration.')
      }
      ...
      // Note: User will be redirected to Google, so this code won't execute
      // The OAuth callback will handle the completion of the flow
  ...
  private setupEventListeners(container: HTMLElement): void {
    ...
    // Google OAuth button
    const oauthButton = container.querySelector('[data-oauth-google]') as HTMLButtonElement
    if (oauthButton) {
      oauthButton.addEventListener('click', this.handleGoogleOAuth.bind(this))
    }
