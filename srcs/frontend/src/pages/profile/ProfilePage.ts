...
export class ProfilePage extends Component<ProfilePageProps, ProfilePageState> {
  ...
  private renderProfileInfo(profile: UserProfile): string {
    ...
    const isCurrentUser = this.isCurrentUserProfile(profile)
      ...
      <section class="bg-green-900/10 border border-green-800 rounded-lg p-6">
        <div class="flex justify-between items-start mb-4">
          ...
          ${isCurrentUser ? 
            ...
            this.renderFriendActions(profile)
          }
        </div>
        ...
        <div class="text-center mb-6">
          <div class="relative w-24 h-24 mx-auto mb-3">
            <div class="w-24 h-24 bg-green-600 rounded-full flex items-center justify-center text-3xl">
            ...
            ${profile.isOnline !== undefined ? `
              <div class="absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-black flex items-center justify-center text-xs ${profile.isOnline ? 'bg-green-500' : 'bg-gray-500'}">
                ${profile.isOnline ? '🟢' : '⚫'}
              </div>
            ` : ''}
          </div>
          ...
          <!-- Online Status Text -->
          ${this.renderOnlineStatus(profile)}
          
          ${isEditing && isCurrentUser ? '<button id="change-avatar-btn" class="text-sm text-green-400 hover:text-green-300">Change Avatar</button>' : ''}
        </div>
        ...
        <div class="space-y-4">
          ...
          ${profile.displayName ? `
            <div>
              <label class="block text-green-300 text-sm mb-1">Display Name</label>
              <p class="text-green-400 font-bold">${profile.displayName}</p>
            </div>
          ` : ''}
          ...
          <div>
            <label class="block text-green-300 text-sm mb-1">Member Since</label>
            <p class="text-green-500 text-sm">${profile.createdAt ? new Date(profile.createdAt).toLocaleDateString() : 'Unknown'}</p>
          </div>

          ${profile.lastSeen && !profile.isOnline ? `
            <div>
              <label class="block text-green-300 text-sm mb-1">Last Seen</label>
              <p class="text-gray-400 text-sm">${new Date(profile.lastSeen).toLocaleString()}</p>
            </div>
          ` : ''}
        ...
        <!-- Friends Quick Actions (for non-current users) -->
        ${!isCurrentUser ? this.renderQuickActions(profile) : ''}
      </section>
    `
  }
  ...
  private async loadProfile(): Promise<void> {
    ...
    try {
      ...
      const { userApiService } = await import('../../services/user/UserService')
  ...
  /**
   * @brief Check if this is the current user's profile
   */
  private isCurrentUserProfile(_profile: UserProfile): boolean {
    // For now, assume it's always the current user's profile
    // In a real implementation, you'd compare with auth store
    return true
  }

  /**
   * @brief Render friend action buttons
   */
  private renderFriendActions(_profile: UserProfile): string {
    return `
      <div class="flex space-x-2">
        <button id="add-friend-btn" class="text-sm px-3 py-1 bg-green-600/20 hover:bg-green-600/30 border border-green-400/50 rounded transition-colors">
          + Add Friend
        </button>
        <button id="message-btn" class="text-sm px-3 py-1 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-400/50 rounded transition-colors">
          💬 Message
        </button>
      </div>
    `
  ...
  /**
   * @brief Render online status
   */
  private renderOnlineStatus(profile: UserProfile): string {
    if (profile.isOnline === undefined) return ''
    ...
    const colorClass = statusColors[statusText as keyof typeof statusColors] || 'text-gray-400'
    
    return `
      <p class="text-sm ${colorClass} capitalize">
        ${statusText}
        ${profile.isOnline ? '🟢' : '⚫'}
      </p>
    `
  ...
  /**
   * @brief Render quick actions for non-current users
   */
  private renderQuickActions(_profile: UserProfile): string {
    return `
      <div class="mt-6 pt-4 border-t border-green-800">
        <h3 class="text-sm font-bold text-green-300 mb-3">Quick Actions</h3>
        <div class="grid grid-cols-2 gap-2">
          <button id="challenge-btn" class="px-3 py-2 text-xs bg-orange-600/20 hover:bg-orange-600/30 border border-orange-400/50 rounded transition-colors">
            🎮 Challenge
          </button>
          <button id="view-games-btn" class="px-3 py-2 text-xs bg-purple-600/20 hover:bg-purple-600/30 border border-purple-400/50 rounded transition-colors">
            📊 View Games
          </button>
        </div>
      </div>
