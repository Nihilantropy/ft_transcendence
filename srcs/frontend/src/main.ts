    ...
    if (event.altKey) {
      ...
      switch (event.key.toLowerCase()) {
        ...
        case 't':
          ...
          import('./test-websocket-integration').then(({ runAllWebSocketTests }) => {
            runAllWebSocketTests().catch(console.error)
          }).catch(console.error)
          handled = true
          break
      }
      ...
      if (handled) {
        event.preventDefault()
        showToast(event.key.toLowerCase() === 't' ? 'Running WebSocket Tests...' : `Navigated via keyboard shortcut`)
      }
    }
